...
import com.md.pay.unionpay.controller.BaseController;
import com.md.pay.unionpay.util.JsonUtils;
import com.md.pay.unionpay.util.ZipUtil;
    ...
    public static void deCodeFileContent(Map<String, String> resData, String fileDirectory, String encoding) {
        ...
        if (null != fileContent && !"".equals(fileContent)) {
            try {
                ...
                File f = new File(filePath.replace(".zip", ""));
                if (f.isDirectory()) {
                    File[] flies = f.listFiles();
                    for (File file2 : flies) {
                        if (file2.getName().contains("IN")) {
                            List<Map<Integer, String>> ls = BaseController.parseZMEFile(file2.getPath());
                            LogUtil.debug(MessageFormat.format("解析对账单zip文件结果：{0}", JsonUtils.toJson(ls)));
                        }
                    }
                }
            } catch (UnsupportedEncodingException e) {
